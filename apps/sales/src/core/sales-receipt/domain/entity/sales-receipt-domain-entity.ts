...
export class SalesReceipt {
  ...
  static create(props: SalesReceiptProps): SalesReceipt {
    ...
    // ✅ Validación de items (mínimo un producto según tu flujo)
  ...
  // Factory method actualizado para recibir los items
  static createNew(
    id_cliente: string,
    id_tipo_venta: number,
    id_tipo_comprobante: number,
    ...
    id_sede_ref: number,
    cod_moneda: string,
    items: SalesReceiptItem[] = [], // ✅ Nuevo parámetro con valor por defecto
  ): SalesReceipt {
  ...
  // ✅ Setter para items (permite al Mapper o Service asignar productos)
  ...
  anular(): SalesReceipt {
    if (this.estado === ReceiptStatus.ANULADO) throw new Error('El comprobante ya está anulado');
    if (this.estado === ReceiptStatus.RECHAZADO) throw new Error('No se puede anular un comprobante rechazado');
  ...
  rechazar(): SalesReceipt {
    if (this.estado === ReceiptStatus.ANULADO) throw new Error('No se puede rechazar un comprobante anulado');
    if (this.estado === ReceiptStatus.RECHAZADO) throw new Error('El comprobante ya está rechazado');
