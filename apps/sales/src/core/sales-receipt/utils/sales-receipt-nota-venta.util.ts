...
export async function buildNotaVentaPdf(
  ...
  function getDescCell(prod: SalesReceiptPdfData['productos'][0]): string {
    ...
    if (tipoPromo === 'PORCENTAJE') {
      ...
      if (base > 0 && montoItem > 0) return `${((montoItem / base) * 100).toFixed(0)}%`;
      return '—';
    }
    ...
    data.productos.forEach((prod, idx) => {
      ...
      cells.forEach((cell, i) => {
        ...
        if (i === 2 && esRemate) {
          ...
          doc.fillColor(C.orange).font('Helvetica-Bold').fontSize(6.5).text(rem.cod_remate, xc, y + 17, { width: 50 });
          doc.fillColor(C.gray).font('Helvetica').fontSize(6.5).text(`Orig: S/${rem.pre_original.toFixed(2)}`, xc + 52, y + 17, { width: 55, strike: true });
          doc.fillColor(C.orange).font('Helvetica-Bold').fontSize(6.5).text(`S/${rem.pre_remate.toFixed(2)}`, xc + 110, y + 17, { width: 50 });
        } else {
          let color = C.black;
          let font  = 'Helvetica';
