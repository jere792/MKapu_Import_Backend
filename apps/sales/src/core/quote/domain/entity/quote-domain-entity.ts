...
export class Quote {
  ...
  constructor(
    ...
    this.fec_emision =
      fec_emision instanceof Date ? fec_emision : new Date(fec_emision);
    this.fec_venc =
      this.fec_venc instanceof Date ? this.fec_venc : new Date(this.fec_venc);
  ...
  aprobar(): void {
    ...
    if (this.estado === 'RECHAZADA')
      throw new Error('No se puede aprobar una cotización rechazada');
    if (!this.activo)
  ...
  rechazar(): void {
    if (this.estado === 'RECHAZADA')
      throw new Error('La cotización ya fue rechazada');
    if (this.estado === 'APROBADA')
      throw new Error('No se puede rechazar una cotización ya aprobada');
  ...
  vencer(): void {
    if (!this.estaVencida())
      throw new Error(
        'Solo cotizaciones expiradas pueden marcarse como vencidas',
      );
    this.estado = 'VENCIDA';
  }

  cambiarEstado(nuevoEstado: QuoteStatus): void {
    const ESTADOS_VALIDOS: QuoteStatus[] = [
      'PENDIENTE',
      'APROBADA',
      'VENCIDA',
      'RECHAZADA',
  ...
  private validarEstadoInicial(): void {
    if (
      !['PENDIENTE', 'APROBADA', 'VENCIDA', 'RECHAZADA'].includes(this.estado)
    )
  ...
  setMontos(subtotal: number, igv: number, total: number): void {
    if (!this.esPendiente())
      throw new Error('Solo puede modificar montos en estado PENDIENTE');
  ...
  setFechaVencimiento(nuevaFecha: Date): void {
    if (!this.esPendiente())
      throw new Error(
        'Solo puede cambiar la fecha de vencimiento en estado PENDIENTE',
      );
