...
function buildQrContent(entity: AccountReceivableOrmEntity): string {
  ...
  const numero = comp ? String(comp.numero).padStart(8, '0') : String(entity.id).padStart(8, '0');
  return [
...
export async function buildAccountReceivablePdf(
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    box(doc, MAR, y - rows.length * 20 - TH, INNER, rows.length * 20 + TH, { stroke: C.border, radius: 3 });
    ...
    totales.forEach(([lbl, val, highlight], i) => {
      ...
      doc.fillColor(fc).font(font).fontSize(highlight ? 9 : 8.5)
        ...
        .text(val, totX + totW / 2, y + (highlight ? 7 : 4), { width: totW / 2 - 10, align: 'right' });
      y += rh;
    });
    ...
    doc.fillColor(C.gray).font('Helvetica').fontSize(7)
      .text(
        ...
        MAR + 84, y + 26, { width: PQW - 92 },
      );

    // Representación impresa
    doc.fillColor(C.gray).font('Helvetica').fontSize(7.5)
      .text('Representación impresa de', xPR + 8, y + 14, { width: PQW - 16, align: 'center' });
    doc.fillColor(C.yellow).font('Helvetica-Bold').fontSize(10)
      .text(tipoDoc.toUpperCase(), xPR + 8, y + 28, { width: PQW - 16, align: 'center' });
    doc.fillColor(C.black).font('Helvetica-Bold').fontSize(8)
      .text(`N° ${docRef}`, xPR + 8, y + 46, { width: PQW - 16, align: 'center' });
    doc.fillColor(C.gray).font('Helvetica').fontSize(7)
      .text(`Generado: ${new Date().toLocaleDateString('es-PE')}`, xPR + 8, y + 60, { width: PQW - 16, align: 'center' })
      .text(`Estado: ${entity.status}`, xPR + 8, y + 72, { width: PQW - 16, align: 'center' });
