...
export class CreditNoteOrmEntity {
    ...
    @Column({ type: 'enum', name: 'tipo_devolucion', enum: CreditNoteBusinessType, default: CreditNoteBusinessType.FULL_REFUND })
    businessType: CreditNoteBusinessType;

    @Column({ type: 'enum', name: 'estado', enum: CreditNoteStatus, default: CreditNoteStatus.ISSUED })
    ...
    @OneToMany(() => CreditNoteItemOrmEntity, (item) => item.creditNote, { cascade: true })
